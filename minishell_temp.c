...
int main(void)
{
    ...
    cmd_node->cmd = strdup("test.txt");  // First command is "echo"
    cmd_node->group_id = 0;          // Example group_id
    cmd_node->type = OUTFILE;         // Assuming "echo" is a built-in comman       // Initially, no next node
    // cmd_node->option = malloc(2 * sizeof(char *));
    // cmd_node->option[0] = strdup("test.txt");
    ...
    cmd_node_3->cmd = strdup("hello");  // Next command is "Hello"
    cmd_node_3->group_id = 0;           // Same group_id as echo
    cmd_node_3->type = STRING;         // You might want to adjust this if "Hello" is a different type
    // cmd_node_3->next = NULL;            // End of the command chain
    cmd_node_3->prev = cmd_node_2;
    cmd_node_3->p_index = 2;
